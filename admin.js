...
function updateDashboard() {
  ...
  // Count distribution across potentially multiple categories
  const catCounts = {};
...
function validateForm(formId) {
  ...
  if (formId === "productForm") {
    ...
    if (selectedFiles.length === 0 && existingImages.length === 0) {
      imgGroup.classList.add("invalid"); isValid = false;
    ...
    if (checkedCats.length === 0) {
        catGroup.classList.add('invalid'); isValid = false;
    } else { catGroup.classList.remove('invalid'); }
  }

  form.querySelectorAll("input, textarea, select").forEach((input) => {
    input.addEventListener("input", () => input.closest(".form-group, .floating-input").classList.remove("invalid"), { once: true });
  });
  return isValid;
}
  ...
  try {
    ...
    snapshot.forEach((docSnap) => {
      ...
      if (data.isActive) {
        ...
        // Add to Multi-select pill container
        multiCatContainer.innerHTML += `
    ...
    updateDashboard();
  } catch (error) { list.innerHTML = '<tr><td colspan="3" style="color:#fca5a5;">Failed to load.</td></tr>'; }
}
...
function renderProductsTable(filterCat = "ALL") {
  ...
  if (filtered.length === 0) {
    list.innerHTML = `<tr><td colspan="5"><div class="empty-state"><i class="ti ti-shopping-bag-x"></i><p>No products match this filter.</p></div></td></tr>`; return;
  }
...
async function loadProducts() {
  try {
    ...
    querySnapshot.forEach((docSnap) => {
      ...
      if(!data.categories && data.category) data.categories = [data.category];
...
function renderPreviews() {
  ...
  existingImages.forEach((url, index) => { previewContainer.innerHTML += `<div class="preview-item"><img src="${url}"><button type="button" class="remove-btn" onclick="window.removeExistingImage(${index})"><i class="ti ti-x"></i></button></div>`; });
...
async function uploadToCloudinary(file) {
  ...
  if (!res.ok) throw new Error("Upload failed"); return (await res.json()).secure_url;
}
...
productForm.addEventListener("submit", async (e) => {
  ...
  const originalHtml = btn.innerHTML;
  btn.innerHTML = '<i class="ti ti-loader ti-spin"></i> Saving...'; btn.disabled = true;
  ...
  try {
    ...
    const data = {
      ...
      categories: selectedCats, // Replaces single category
...
window.viewProduct = (id) => {
  ...
  let thumbsHtml = "";
  const mainImg = p.images && p.images.length > 0 ? p.images[0] : "https://via.placeholder.com/400";
...
window.editProduct = (id) => {
  ...
  // Clear and Select multiple categories
  ...
  catsToSelect.forEach(c => {
      const cb = document.querySelector(`input[name="productCategories"][value="${c}"]`);
...
async function loadEstimations() {
  try {
    ...
    estimationsCache = [];
    snapshot.forEach((docSnap) => {
      const data = docSnap.data(); data.id = docSnap.id; estimationsCache.push(data);
    });
...
window.saveEstimation = async () => {
  ...
  const estData = { customerId: cust.id, customerName: cust.companyName, date: new Date().toISOString(), products: currentEstProducts };
...
window.printEstimation = (id) => {
  ...
  est.products.forEach((p, index) => {
    ...
    // Support multi-category in print
